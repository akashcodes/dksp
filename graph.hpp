...
class Edge {
    ...
    Edge(){};
    Edge(Vertex*, Vertex*, long long);
    void update_weight(long long);
};
...
*/
Edge::Edge(Vertex* source, Vertex* destination, long long weight_param) {
    this->source = source;
...
void Edge::update_weight(long long new_weight) {
...
struct edge_weight_comparator {
    bool operator() (const std::pair<long long, Vertex*>& a, const std::pair<long long, Vertex*>& b) const {
...
class Graph {
    ...
    std::map<long long, Vertex*> vertices;
    ...
    std::unordered_set<long long> boundary_vertices;
    ...
    Graph(std::vector<std::vector<long long> > const&, std::vector<long long> const&);

    std::vector<std::vector<Vertex*> > get_k_bounding_paths(Vertex*, Vertex*, long long);

    void initialise_bounding_paths(long long);
    ...
    Edge* get_edge(long long, long long);

    // returns true if the edge weight is updated successfully
    bool update_edge_weight(long long u, long long v, long long);

    bool update_edge_weight(Edge*, long long);

    void print_neighbours(long long);
...
Graph::Graph(std::vector<std::vector<long long> > const& edges_param, std::vector<long long> const& boundary_vertices_param) {
    for(long long i = 0; i < (long long) edges_param.size(); i++) {
        //std::cout<<"\n";
        long long u = edges_param[i][0];
        long long v = edges_param[i][1];
        long long w = edges_param[i][2];
    ...
    for(long long i = 0; i < (long long) boundary_vertices_param.size(); i++) {
        long long u = boundary_vertices_param[i];
...
*/
Edge* Graph::get_edge(long long u, long long v) {
 ...
 * - complete the method
*/
std::vector<std::vector<Vertex*> > Graph::get_k_bounding_paths(Vertex* source, Vertex* destination, long long k) {
    ...
    for(long long ki = 1; ki < k; ki++) {

        for(long long i = 0; i < paths[ki-1].size()-2; i++) {
            ...
            if(spur_path.size() >= 2) {
                ...
                bool existing = true;
                long long cpn = candidate_paths.size();
...
std::vector<Vertex*> Graph::get_shortest_path(Vertex* source, Vertex* destination) {
    ...
    std::priority_queue<std::pair<long long, Vertex*>, std::vector<std::pair<long long, Vertex*>>, edge_weight_comparator> open;
    ...
    //predecessor[source] = new Vertex();;
    while(!open.empty()) {
        std::pair<long long, Vertex*> pq_pair = open.top();
        open.pop();
...
bool Graph::update_edge_weight(Edge* edge, long long new_weight) {
...
bool Graph::update_edge_weight(long long u, long long v, long long new_weight) {
...
void Graph::print_neighbours(long long node) {
...
long long path_distance(std::vector<Vertex*>& path, std::map<Vertex*, std::map<Vertex*, Edge*> >& adjacency_list) {
